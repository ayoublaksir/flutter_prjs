...
class AdsConfig {
  ...
  static Future<void> initializeRemoteConfig() async {
    ...
    try {
      ...
      await _remoteConfig!.setDefaults({
        ...
        // === REWARDED AD LIMITS (FIREBASE CONTROLLED) ===
        'daily_routine_limit':
            '3', // FIREBASE CONTROLLED: Free routines per day
        'daily_product_limit':
            '3', // FIREBASE CONTROLLED: Free products per day
        'daily_tip_limit': '5', // FIREBASE CONTROLLED: Free tips per day
        'rewarded_unlock_count':
            '3', // FIREBASE CONTROLLED: Extra items unlocked per rewarded ad

        // === AD FREQUENCY CONTROL (FIREBASE CONTROLLED) ===
        'interstitial_cooldown_minutes':
            '10', // FIREBASE CONTROLLED: Minutes between interstitial ads
        'banner_refresh_minutes':
            '2', // FIREBASE CONTROLLED: Minutes before banner auto-refresh
        'banner_cache_minutes':
            '5', // FIREBASE CONTROLLED: Minutes to cache banner ads
  ...
  // ==========================================================================
  // REWARDED AD LIMITS GETTERS
  // ==========================================================================

  /// Get daily limits from Firebase Remote Config
  static int get dailyRoutineLimit {
    final limit = getIntValue('daily_routine_limit');
    return limit > 0 ? limit : 3; // Default to 3 if invalid
  }

  static int get dailyProductLimit {
    final limit = getIntValue('daily_product_limit');
    return limit > 0 ? limit : 3; // Default to 3 if invalid
  }

  static int get dailyTipLimit {
    final limit = getIntValue('daily_tip_limit');
    return limit > 0 ? limit : 5; // Default to 5 if invalid
  }

  static int get rewardedUnlockCount {
    final count = getIntValue('rewarded_unlock_count');
    return count > 0 ? count : 3; // Default to 3 if invalid
  }

  // ==========================================================================
  // AD FREQUENCY CONTROL GETTERS
  // ==========================================================================

  /// Get interstitial cooldown duration from Firebase Remote Config
  static Duration get interstitialCooldownDuration {
    final minutes = getIntValue('interstitial_cooldown_minutes');
    return Duration(
        minutes: minutes > 0 ? minutes : 10); // Default to 10 minutes
  }
